...
def get_fuzzing_headers(payload):
    ...
    if "Referer" in fuzzing_headers:
        fuzzing_headers["Referer"] = 'https://fuzzing_headers["{}"]'.format(Referer)
...
class Interactsh:
    ...
    def __init__(self, token="", server=""):
        ...
        self.domain = '{}.{}'.format(guid,self.server)
    ...
    def register(self):
        ...
        res = self.session.post(
            "https://{}/register".format(self.server), headers=self.headers, json=data, timeout=30)
        if 'success' not in res.text:
            raise Exception("Can not initiate interact.sh DNS callback client")

    def pull_logs(self):
        result = []
        url = "https://{}/poll?id={}&secret={}".format(self.server,self.correlation_id,self.secret)
    ...
    def __parse_log(self, log_entry):
        new_log_entry = {"timestamp": log_entry["timestamp"],
                         "host": '{}.{}'.format(log_entry["full-id"],self.domain),
...
def parse_url(url):
    ...
    return({"scheme": scheme,
            "site": "{}://{}".format(scheme,urlparse.urlparse(url).netloc),
...
def scan_url(url, callback_host):
    ...
    if args.waf_bypass_payloads:
        payloads.extend(generate_waf_bypass_payloads('{}.{}'.format(parsed_url["host"],callback_host), random_string))

    if args.cve_2021_45046:
        cprint("[•] Scanning for CVE-2021-45046 (Log4j v2.15.0 Patch Bypass - RCE)", "yellow")
        payloads = get_cve_2021_45046_payloads('{}.{}'.format(parsed_url["host"],callback_host), random_string)

    for payload in payloads:
        cprint("[•] URL: {} | PAYLOAD: {}".format(url,payload), "cyan")
...
def main():
    ...
    if args.custom_dns_callback_host:
        cprint("[•] Using custom DNS Callback host [{}]. No verification will be done after sending fuzz requests.".format(args.custom_dns_callback_host))
        dns_callback_host = args.custom_dns_callback_host
    else:
        cprint("[•] Initiating DNS callback server ({}).".format(args.dns_callback_provider))
